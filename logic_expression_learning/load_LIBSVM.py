...
def load_file(filename: str, n_samples: int) -> Tuple[np.ndarray, np.ndarray]:
    np.random.seed(2)
    if not os.path.isfile(f"../dataset/{filename}"):
        download_from_libsvm(filename)
